...
func sessionRowsSQL(where string) string {
	columns := []string{"input_tokens", "cached_input_tokens", "cache_write_input_tokens", "cache_write_5m", "cache_write_1h", "output_tokens", "reasoning_output_tokens", "total_tokens"}
	...
	for _, c := range []string{"claude_home", "thread_title", "project_path", "model", "source", "agent_type"} {
	...
	return `WITH page AS MATERIALIZED (
		...
		COALESCE(s.rollout_path,''),COALESCE(NULLIF(s.claude_home,''),a.claude_home,''),
