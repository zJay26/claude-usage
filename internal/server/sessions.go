...
func makeSessionQueryKey(revision uint64, filter model.Filter, limit, offset int, compact bool) sessionQueryKey {
	return sessionQueryKey{
		Homes: strings.Join(filter.Homes, "\x00"), Revision: revision, Mode: filter.Mode, CostBasis: filter.CostBasis, SinceUnix: timeCacheKey(filter.Since), UntilUnix: timeCacheKey(filter.Until),
