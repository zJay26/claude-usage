...
import (
	...
	usageweb "github.com/zJay26/claude-usage/internal/web"
)
...
func (s *Server) handleExport(w http.ResponseWriter, r *http.Request) {
	...
	case "csv":
		...
		w.Header().Set("Content-Disposition", `attachment; filename="claude-usage-`+stamp+`.csv"`)
		...
		_ = writer.Write([]string{
			...
			"output", "reasoning_output", "total", "provenance", "confidence", "claude_home", "service_mode", "service_tier", "mode_source", "mode_assumed",
			"cache_write_5m", "cache_write_1h", "cache_write_unknown", "sources", "iteration_type",
		})
		err = s.forEachEvent(r.Context(), filter, func(event model.UsageEvent) error {
			return writer.Write([]string{
				...
				csvText(event.Provenance), csvText(event.Confidence), csvText(event.ClaudeHome),
				...
				strconv.FormatInt(event.Usage.CacheWrite5m, 10), strconv.FormatInt(event.Usage.CacheWrite1h, 10),
				strconv.FormatInt(event.Usage.CacheWriteInput-event.Usage.CacheWrite5m-event.Usage.CacheWrite1h, 10),
				csvText(strings.Join(event.Sources, " | ")), csvText(event.IterationType),
			})
		})
		writer.Flush()
		if err == nil {
	...
	case "json":
		...
		w.Header().Set("Content-Disposition", `attachment; filename="claude-usage-`+stamp+`.json"`)
		w.Write([]byte("[\n"))
		first := true
