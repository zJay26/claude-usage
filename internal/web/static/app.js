...
const i18n = window.ClaudeUsageI18n;
const t = (key, values) => i18n.t(key, values);

const FILTER_FIELDS = {
  home: { selector: "#filterHomes", labelKey: "filter.home" },
...
const DISPLAY_PREFERENCES_KEY = "claude-usage-display-preferences";
...
function loadDisplayPreferences() {
  ...
  const legacyTheme = readStorage("claude-usage-theme");
...
function applyDisplayPreferences({ persist = false } = {}) {
  ...
  if (persist) {
    ...
    if (displayPreferences.theme === "system") removeStorage("claude-usage-theme");
    else writeStorage("claude-usage-theme", displayPreferences.theme);
  }
  syncSettingsForm();
}
...
function resetDisplayPreferences() {
  ...
  removeStorage("claude-usage-theme");
...
const emptyUsage = () => ({ input: 0, cached_input: 0, cache_write_input: 0, cache_write_5m: 0, cache_write_1h: 0, output: 0, reasoning_output: 0, total: 0 });
...
async function api(path, options = {}) {
  ...
  const cacheable = method === "GET" && !path.startsWith("/api/v1/sources") && !path.startsWith("/api/v1/status") && !path.startsWith("/api/v1/warnings") && !path.startsWith("/api/v1/updates");
...
function filterQuery(extra = {}, filters = state.filters) {
  ...
  for (const [key, value] of Object.entries(values)) {
    ...
    if (Array.isArray(value)) { for (const item of value) query.append(key, item); continue; }
...
function apiURL(path, extra = {}, filters = state.filters) {
  const query = filterQuery({ ...extra, ...(["/api/v1/cost-estimate", "/api/v1/sessions", "/api/v1/session-estimates", "/api/v1/session-tree"].includes(path) ? { cost_basis: "claude_fast_weighted" } : {}) }, filters);
...
async function loadStatus() {
  ...
  for (const home of status.claude_homes || []) {
...
async function loadSourceSettings() {
  const data = await loadSourceOptions();
  $("#sourcePaths").value = (data.extra_claude_homes || []).join("\n");
  $("#discoverWSL").checked = Boolean(data.auto_discover_wsl);
  $("#startWSL").checked = Boolean(data.auto_start_wsl);
  $("#sourceList").innerHTML = (data.sources || []).map(source=>`<article class="source-row"><div class="source-row-head"><strong>${escapeHTML(source.label || source.platform)}</strong><span>${escapeHTML(t("sources." + source.state))}</span></div><code>${escapeHTML(source.path || source.id)}</code>${source.error ? `<p class="source-error">${escapeHTML(source.error)}</p>` : ""}</article>`).join("") || `<p class="dialog-intro">${escapeHTML(t("sources.empty"))}</p>`;
}
...
function syncFilterForm() {
  for (const [key, meta] of Object.entries(FILTER_FIELDS)) {
    ...
    if (el.multiple) [...el.options].forEach(option => option.selected = (state.filters[key] || []).includes(option.value));
...
function applyFiltersFromForm() {
  ...
  for (const [key, meta] of Object.entries(FILTER_FIELDS)) {
    const el = $(meta.selector);
    const value = el.multiple ? [...el.selectedOptions].map(o => o.value) : el.value;
...
function renderOverviewSummary(summary) {
  ...
  $("#overviewTokenBreakdown").innerHTML = [
    ["Input (total)", usage.input], ["Cached", usage.cached_input], ["Write 5m", usage.cache_write_5m], ["Write 1h", usage.cache_write_1h],
    ...(usage.cache_write_input - (usage.cache_write_5m || 0) - (usage.cache_write_1h || 0) > 0 ? [["Write ?", usage.cache_write_input - (usage.cache_write_5m || 0) - (usage.cache_write_1h || 0)]] : []),
    ["Output", usage.output], ["Thinking ⊂ Output", usage.reasoning_output]
...
function setExportLinks() {
  ...
  $("#exportCsv").href = window.ClaudeUsageDemo?.createExportURL?.("csv", csvPath) || csvPath;
  $("#exportJson").href = window.ClaudeUsageDemo?.createExportURL?.("json", jsonPath) || jsonPath;
}
...
function renderCatalog(catalog) {
  $("#pricingCatalog").innerHTML = `<div class="catalog-row header"><span>${escapeHTML(t("pricing.catalogModel"))}</span><span>Input</span><span>Cached</span><span>Write 5m</span><span>Write 1h</span><span>Output</span></div>${catalog.map((entry) => `<div class="catalog-row"><a href="${escapeHTML(entry.source)}" target="_blank" rel="noreferrer">${escapeHTML(entry.display_name)}</a><span>${escapeHTML(entry.input_usd_per_million)}</span><span>${escapeHTML(entry.cached_input_usd_per_million)}</span><span>${escapeHTML(entry.cache_write_input_usd_per_million || "—")}</span><span>${escapeHTML(entry.cache_write_1h_usd_per_million || "—")}</span><span>${escapeHTML(entry.output_usd_per_million)}</span></div>`).join("")}`;
}
  ...
  card.innerHTML = `<div class="override-card-head"><div><strong>${escapeHTML(model)}</strong><small>${observedTokens == null ? escapeHTML(t("pricing.localCustom")) : escapeHTML(t("pricing.observed", { tokens: fullToken(observedTokens) }))}</small></div><button class="remove-override pressable" type="button">${escapeHTML(t("action.remove"))}</button></div>
    <div class="override-mode">
      ...
      <div class="custom-rate-grid">
        ...
        <label>Write 5m<input data-rate="cache_write_input_usd_per_million" inputmode="decimal" value="${escapeHTML(override?.cache_write_input_usd_per_million || "")}" placeholder="USD / 1M"></label>
        <label>Write 1h<input data-rate="cache_write_1h_usd_per_million" inputmode="decimal" value="${escapeHTML(override?.cache_write_1h_usd_per_million || "")}" placeholder="USD / 1M"></label>
...
function collectPricingOverrides() {
  ...
  for (const card of $$('[data-pricing-model]', $("#pricingOverrideList"))) {
    ...
    for (const key of ["input_usd_per_million", "cached_input_usd_per_million", "cache_write_input_usd_per_million", "cache_write_1h_usd_per_million", "output_usd_per_million"]) {
...
function setupEvents() {
  ...
  $("#filterButton").addEventListener("click", () => {
    ...
    loadFilterOptions();
    loadSourceOptions().catch(error => toast(error.message, true));
  });
  ...
  $("#sourcesButton").addEventListener("click", async () => {
    openDialog($("#sourcesDialog"));
    try { await loadSourceSettings(); } catch (error) { toast(error.message, true); }
  });
  $("#saveSources").addEventListener("click", async () => {
    const button = $("#saveSources"); button.disabled = true;
    try {
      await api("/api/v1/sources", {method:"PUT", headers:{"Content-Type":"application/json"}, body:JSON.stringify({extra_claude_homes:$("#sourcePaths").value.split(/\r?\n/).map(s=>s.trim()).filter(Boolean), auto_discover_wsl:$("#discoverWSL").checked, auto_start_wsl:$("#startWSL").checked})});
      closeDialog($("#sourcesDialog"));
      await runScan(false);
  ...
  window.addEventListener("claude-usage-locale-change", async () => {
    syncSettingsForm();
    renderFilterOptions();
    renderFilterChips();
    try {
...
async function boot() {
  applyDisplayPreferences();
  i18n.applyStatic();
  if (window.CLAUDE_USAGE_DEMO || window.ClaudeUsageDemo) $("#demoBanner").classList.remove("hidden");
  setupEvents();
  renderFilterChips();
