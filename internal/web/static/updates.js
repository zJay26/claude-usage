(() => {
  ...
  function render() {
    ...
    if (/^https:\/\/github\.com\/zJay26\/claude-usage\/releases(?:\/tag\/v\d+\.\d+\.\d+)?$/.test(status.release_url)) $("#updateReleaseLink").href = status.release_url;
    const dismissed = readStorage("claude-usage-update-dismissed") === status.latest_version;
  ...
  $("#dismissUpdate").addEventListener("click", () => { writeStorage("claude-usage-update-dismissed", status.latest_version); render(); });
  ...
  window.addEventListener("claude-usage-locale-change", render);
