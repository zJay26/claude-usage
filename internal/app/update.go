...
func (c CLI) applyUpdate(args []string) error {
	...
	root := filepath.Join(paths.StateDir, ".claude-usage-updates")
...
// Login launchers set CLAUDE_USAGE_HOME even for the default installation.
...
func recordInstallation(paths config.Paths) error {
	return updater.WriteJSON(filepath.Join(paths.StateDir, ".claude-usage-install.json"), map[string]string{"executable": paths.InstalledEXE})
}
func updatePaths(paths config.Paths) config.Paths {
	var record struct {
		Executable string `json:"executable"`
	}
	data, err := os.ReadFile(filepath.Join(paths.StateDir, ".claude-usage-install.json"))
...
func runUpdate(paths config.Paths, dir string, job updateJob) (string, error) {
	...
	if err != nil || !strings.HasPrefix(string(output), "claude-usage "+job.Version+" ") {
