...
func TestLaunchAgentEscapesPathsAndSeparatesUpdateHelper(t *testing.T) {
	t.Setenv("CLAUDE_CONFIG_DIR", "/tmp/claude & <source>")
	for _, managed := range []bool{true, false} {
		body, err := launchPlist("test.label", "/tmp/a & b/claude-usage", "/tmp/state <test>", []string{"daemon"}, managed)
		if err != nil {
			t.Fatal(err)
		}
...
func TestDarwinStopRejectsAnotherExecutable(t *testing.T) {
	...
	if err := stopDarwinProcess(cmd.Process.Pid, "/tmp/claude-usage"); err == nil {
...
func TestDarwinWaitDoesNotSignalReusedPID(t *testing.T) {
	...
	t.Cleanup(func() { _ = cmd.Process.Kill(); _ = cmd.Wait() })
	if err := waitDarwinProcessExit(cmd.Process.Pid, "/tmp/previous/claude-usage"); err != nil {
		t.Fatal(err)
	}
