...
func ManagedUpdateService() bool {
	...
	return strings.Contains(string(data), "/claude-usage.service")
}
func StartUpdateHelper(executable, job, stateDir string, managed bool) error {
	...
	// keeps the helper alive when claude-usage.service is stopped.
	args := []string{"--user", "--collect", "--unit", fmt.Sprintf("claude-usage-update-%d", time.Now().UnixNano()), "--setenv=CLAUDE_USAGE_HOME=" + stateDir}
	for _, name := range []string{"CLAUDE_CONFIG_DIR", "XDG_CONFIG_HOME", "XDG_DATA_HOME"} {
...
func StopForUpdate(executable, stateDir string, managed bool) error {
	...
	if managed {
		if output, err := exec.Command("systemctl", "--user", "stop", "claude-usage.service").CombinedOutput(); err != nil {
...
func StartAfterUpdate(executable string, managed bool) error {
	...
	output, err := exec.Command("systemctl", "--user", "start", "claude-usage.service").CombinedOutput()
	if err != nil {
