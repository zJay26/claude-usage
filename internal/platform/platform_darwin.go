...
const launchAgentLabel = "com.zjay.claude-usage"
...
func launchPlist(label, executable, stateDir string, args []string, keepAlive bool) (string, error) {
	...
	b.WriteString("</array><key>EnvironmentVariables</key><dict><key>CLAUDE_USAGE_HOME</key>" + plistString(stateDir))
	if keepAlive {
		b.WriteString("<key>CLAUDE_USAGE_LAUNCHD</key><string>1</string>")
	}
	for _, name := range []string{"CLAUDE_CONFIG_DIR", "CLAUDE_USAGE_TIMEZONE", "TZ"} {
