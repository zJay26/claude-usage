...
func TestDownloadUsesSelectedDirectoryAndPrivateStaging(t *testing.T) {
	...
	private := <-applied
	if !strings.HasPrefix(private, filepath.Join(m.stateDir, ".claude-usage-updates")+string(os.PathSeparator)) {
		t.Fatal("helper staging escaped state directory", private)
	}
	s := m.Status()
	if filepath.Dir(filepath.Dir(s.LastDownload)) != s.DownloadDir || filepath.Base(s.LastDownload) != m.asset || !strings.HasPrefix(filepath.Base(filepath.Dir(s.LastDownload)), "claude-usage-v2.5.0-") {
		t.Fatal(s)
	}
