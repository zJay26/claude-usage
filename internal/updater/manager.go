...
func (m *Manager) file() string { return filepath.Join(m.stateDir, ".claude-usage-updates.json") }
...
func ResultPath(dir string) string { return filepath.Join(dir, ".claude-usage-update-result.json") }
...
func (m *Manager) prepare(ctx context.Context, r Release) error {
	...
	downloadRun, err := os.MkdirTemp(downloadDir, "claude-usage-"+r.Tag+"-")
	if err != nil {
		return err
	}
	...
	root := filepath.Join(m.stateDir, ".claude-usage-updates")
