...
try {
    ...
    foreach ($Target in $Targets) {
        ...
        $Name = "claude-usage-$($Target.OS)-$($Target.Arch)$($Target.Suffix)"
        $Output = Join-Path $Dist $Name
        $Artifacts += $Output
        $Ldflags = "-s -w -X github.com/zJay26/claude-usage/internal/app.Version=$Version -X github.com/zJay26/claude-usage/internal/app.Commit=$Commit -X github.com/zJay26/claude-usage/internal/app.BuildDate=$BuildDate"
        & $Go build -trimpath -buildvcs=false -ldflags $Ldflags -o $Output ./cmd/claude-usage
