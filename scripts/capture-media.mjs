...
const tempDir = await mkdtemp(path.join(tmpdir(), "claude-usage-media-"));
const viewport = { width: 1440, height: 1000 };
const fixedTime = new Date("2026-09-24T16:30:00+08:00");
...
const baseURL = `http://127.0.0.1:${port}/claude-usage/`;
...
async function recordDemo(browser, locale) {
  ...
  async function move(locator) {
    ...
    const duration = Math.min(850, Math.max(420, Math.hypot(target.x - pointer.x, target.y - pointer.y) * .72));
  ...
  async function click(selector, pause = 700) {
    ...
    await locator.click();
    await delay(Math.max(pause, 650));
  }
  ...
  try {
    ...
    await page.locator("#rangeStart").fill("2026-09-24T09:00");
    await delay(900);
    await move(page.locator("#rangeEnd"));
    await page.locator("#rangeEnd").fill("2026-09-24T10:30");
    await delay(900);
    ...
    await click('[data-calendar-date="2026-09-23"]', 750);
    ...
    await page.locator("#sessionSearch").pressSequentially("local", { delay: 140 });
    await delay(950);
    ...
    await delay(650);
    await move(page.locator("#filterModel"));
    await page.locator("#filterModel").selectOption("claude-opus-5-5");
    await delay(650);
    await click("#applyFilters", 650);
    await expect(page.locator(".filter-chip")).toHaveCount(2);
    await chapter("combined-fast-model-filter");
    ...
    await click("#exportCsv", 800);
    await (await download).saveAs(path.join(chapterDir, "synthetic-export.csv"));
    ...
    await scrollTo("#sourcesButton", viewport.height - 70);
    await click("#sourcesButton", 800);
    await expect(page.locator("#sourceList .source-row")).toHaveCount(2);
    await chapter("windows-wsl-sources");
    await click("#sourcesDialog .dialog-close", 650);
    ...
    await click("#settingsButton", 750);
    await click('label:has([data-setting="fontSize"][value="large"])', 750);
    await chapter("display-preferences");
    await click('label:has([data-setting="fontSize"][value="comfortable"])', 700);
    await click("#settingsDialog .dialog-close", 650);
    ...
    const mp4 = path.join(mediaDir, `claude-usage-demo-${locale === "en" ? "en" : "zh"}.mp4`);
    ...
    const gif = path.join(mediaDir, `claude-usage-demo${locale === "en" ? "-en" : ""}.gif`);
    ...
    for (const file of [gif, mp4]) {
      ...
      if (length < 10_000 || length > 25_000_000) throw new Error(`Unexpected media size: ${file}: ${length}`);
    }
    await writeFile(path.join(chapterDir, "chapters.json"), JSON.stringify({ locale, viewport, contentSeconds, gifSeconds, gifMaxFPS: 20, playbackSpeed: 1, pointerMillis: [420,850], minimumClickPauseMillis:650, chapterReadingMillis:2000, typingMillisPerKey:140, warnings, errors, chapters }, null, 2) + "\n");
  } finally {
    await context.close();
  }
}
...
const server = spawn(process.execPath, ["scripts/serve-static.mjs", "dist/pages", String(port), "claude-usage"], { cwd: repoRoot, stdio: "ignore", windowsHide: true });
try {
  await waitForServer();
  ...
  if (!path.resolve(tempDir).startsWith(tempRoot) || !path.basename(tempDir).startsWith("claude-usage-media-")) throw new Error("Unexpected capture cleanup path");
