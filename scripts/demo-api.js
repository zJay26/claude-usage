(function (root) {
  ...
  const agents = [{ key: "main", share: .75, events: 80, sessions: 20 }, { key: "subagent", share: .25, events: 34, sessions: 6 }];
  const sources = [
  ...
  async function syntheticFetch(input, init = {}) {
    ...
    if (endpoint === "/api/v1/status") return jsonResponse({
      version: "0.1.0-demo", scanning: false,
      status: {
        ...
        claude_homes: [{ path: "synthetic://claude-home", last_scan: now.toISOString(), files_scanned: 26 }]
      }
    });
