...
for target in windows/amd64 windows/arm64 linux/amd64 linux/arm64 darwin/amd64 darwin/arm64; do
  ...
  output="$dist/claude-usage-$os-$arch$suffix"
  CGO_ENABLED=0 GOOS="$os" GOARCH="$arch" "$go_bin" build \
    -trimpath -buildvcs=false \
    -ldflags "-s -w -X github.com/zJay26/claude-usage/internal/app.Version=$version -X github.com/zJay26/claude-usage/internal/app.Commit=$commit -X github.com/zJay26/claude-usage/internal/app.BuildDate=$build_date" \
    -o "$output" ./cmd/claude-usage
done
...
(
  ...
  sha256sum \
    ...
    claude-usage-windows-arm64.exe > SHA256SUMS
)
