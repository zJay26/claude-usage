...
test.beforeAll(async () => {
  ...
  baseURL = `http://127.0.0.1:${port}/claude-usage/`;
  server = spawn(process.execPath, ["scripts/serve-static.mjs", "dist/pages", String(port), "claude-usage"], { stdio: "ignore", windowsHide: true });
...
test("Pages subpath loads the canonical UI with synthetic-only APIs", async ({ page }) => {
  ...
  const health = await page.evaluate(async () => ({
    ...
    cost: await (await fetch("api/v1/cost-estimate?cost_basis=claude_fast_weighted")).json()
  }));
...
test("synthetic modes, Fast pricing, and export follow the accounting contract", async ({page}) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    const basic = await api("cost-estimate?model=claude-opus-5-5&mode=fast");
    const weighted = await api("cost-estimate?model=claude-opus-5-5&mode=fast&cost_basis=claude_fast_weighted");
  ...
  expect(result.weighted.summary.coverage_ratio).toBe(1);
...
test("synthetic catalog supports Claude aliases and Fast estimates", async ({ page }) => {
  await page.goto(`${baseURL}?lang=en&scenario=diagnostics`, { waitUntil: "networkidle" });
  await page.locator("#pricingButton").click();
  await page.locator(".catalog-disclosure summary").click();
  const card = page.locator('[data-pricing-model="claude-auto-review"]');
  await card.locator("[data-rate-mode]").selectOption("alias");
  for (const item of [
    { model: "claude-opus-5-5", display: "Claude Opus 5.5", rates: ["4", "0.2", "5", "8", "20"] },
    { model: "claude-opus-5", display: "Claude Opus 5", rates: ["5", "0.5", "6.25", "10", "25"] }
  ]) {
    ...
    const result = await page.evaluate(async (model) => {
      await fetch("api/v1/pricing/overrides", { method: "PUT", body: JSON.stringify({ overrides: { "claude-auto-review": { alias_of: model } } }) });
      const base = await (await fetch("api/v1/cost-estimate?model=claude-auto-review&mode=fast")).json();
      const fast = await (await fetch("api/v1/cost-estimate?model=claude-auto-review&mode=fast&cost_basis=claude_fast_weighted")).json();
      return { base: base.summary, fast: fast.summary };
    }, item.model);
    expect(Number(result.base.usd)).toBeGreaterThan(0);
    expect(Number(result.fast.usd)).toBeCloseTo(Number(result.base.usd) * 2, 7);
...
test("language priority, persistence, ARIA, pricing, scan, theme, and mobile layout work", async ({ page }) => {
  ...
  await page.locator("#pricingButton").click();
  await page.locator("#newOverrideModel").fill("claude-auto-review");
  await page.locator("#addOverride").click();
  const card = page.locator('[data-pricing-model="claude-auto-review"]');
  await card.locator("[data-rate-mode]").selectOption("alias");
  await card.locator("[data-alias]").selectOption("claude-opus-5-5");
